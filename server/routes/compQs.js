...
const { compQ, validate } = require("../Models/compQ");
const bcrypt = require("bcrypt");

router.get('/', async(req,res) => {
    try{
        const result = await compQ.find();
		if (!result) {
            res.json({
                status: "FAILED",
...
router.get('/:id', async(req,res) => {
    try{
		...
		const result = await compQ.findById(_id);
        if (!result) {
            res.json({
                status: "FAILED",
...
router.post("/", async (req, res) => {
	try {
		...
		const result = await new compQ(req.body).save();
...
router.patch('/:id', async(req,res)=> {
    try{
		const _id = req.params.id;
		const question = await compQ.findById(_id);
		...
		const result = await compQ.findByIdAndUpdate(_id,req.body,{new: true});
        console.log(result)
        if (!result) {
            res.json({
                status: "FAILED",
...
router.delete("/:id", async (req, res) => {
    try {
        const _id = req.params.id;
        const result = await compQ.findByIdAndDelete(_id);
        if (!result) {
            res.json({
                status: "FAILED",
