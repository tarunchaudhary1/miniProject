...
const { mcqResult, validate } = require("../models/mcqResult");
const bcrypt = require("bcrypt");

router.get('/', async(req,res) => {
    try{
        const obj = await mcqResult.find();
		if (!obj) {
            res.json({
                status: "FAILED",
...
router.get('/:id', async(req,res) => {
    try{
		...
		const obj = await mcqResult.findById(_id);
        if (!obj) {
            res.json({
                status: "FAILED",
...
router.post("/", async (req, res) => {
	try {
		...
		const obj = await new mcqResult(req.body).save();
...
router.patch('/:id', async(req,res)=> {
    ...
    try {
		const _id = req.params.id;
		const res_obj = await mcqResult.findById(_id);
        ...
        //const obj = await mcqResult.findByIdAndUpdate(_id, { '$push': { 'mcqQs[0]': req.body.mcqQs } })

		const obj = await mcqResult.findByIdAndUpdate(_id, req.body, {new: true});
        if (!obj) {
            res.json({
                status: "FAILED",
...
router.delete("/:id", async (req, res) => {
    try {
        const _id = req.params.id;
        const obj = await mcqResult.findByIdAndDelete(_id);
        if (!obj) {
            res.json({
                status: "FAILED",
